...
def main() -> None:
    ...
    lines = [
        ...
        "",
        (f"**Fitted over points at or above {args.fit_from:,.0f} supervised tokens.** "
         "A restriction here is POST-HOC:" if args.fit_from else
         "**Fitted over every point.**"),
        ("the registered run fitted every point and returned a negative intercept"
         if args.fit_from else "this is the registered form."),
        "",
