...
example run alone. That identity holds per example, not per optimizer step, and
the literature nonetheless advises inheriting the learning rate — advice we can
find asserted but never swept. We sweep it: a 2x2 factorial of batch size
against step count, on two instruction corpora and on subsets spanning packing
factors from 2.7x to 7.8x, at three model sizes spanning 17x.
...
A control identifies the mechanism. A padded run whose gradient accumulation is
raised until it matches a packed step on supervised tokens, on examples and on
data seen — differing only in representation, at several times the forward-pass
cost — finds the same optimum, against the three- to four-and-a-half-fold shift
a rule keyed on forward-pass rows predicts. Packing acts on the learning rate
purely as a change of batch size.
...
ruled out and the packing ratio is unsupported once the seed spread is carried
onto the exponent; what survives is the scale of the run, confirmed by three
predictions registered before the deciding runs existed and by the two
...
0.686 and the *random* third at 0.670, inside either one's bound. They share
...
batch term drops to 0.48 of its value and the step term to 0.16. **The scale
...
| Alpaca, random ninth | 5,652 | 4.53x | 0.385 ± 0.100 |
...
0.108, and by 0.071 against 0.136, or 0.10x and 0.53x of their own noise. One
...
cell here except the 124M ninth, which has two for the reason in Appendix C;
none of the fourteen cells added at 30M and 7M lost a seed:
...
2.5x at 7M. Against a ninth, where we have two of the three sizes, it is 4.1x
...
| --- | --- |
| Alpaca ninth, 124M | 16% **below** the floor |
...
the exponent is 0.385, 0.456 and 0.456 — under the 0.5 that `sqrt(p)` assumes.
