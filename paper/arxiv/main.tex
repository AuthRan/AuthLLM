...
\caption{Left: the exponent against the base model's validation perplexity. The 124M series spans the whole quality range these three models cover and stays flat, while the 7M model sits at the same perplexity as its worst point and more than half an exponent above it. Right: the optima themselves, on the same axis. Both arms rise together as the base model gets worse -- by three to four times over this range -- while the distance between them, the only quantity this paper measures, is left where it was. Error bars are the seed bound of \S{}4.5; perplexities are read from each model's own pretraining log.}
\end{figure}
