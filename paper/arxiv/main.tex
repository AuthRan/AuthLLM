...
Every number in this paper comes from one of five ledgers, one per base model, each a row per run --- dataset, cell, learning rate, seed, final and best validation loss, the step the best arrived at, and wall time. The sweep is resumable and skips rows already present, so each file is both the output and the ledger.
