...
| M13 | Optional public-checkpoint loader (inference-only) | Loaded model reproduces expected reference logits on a fixed prompt |
| M14 | Clean inference API (`engine.py`) | `AshuGPT.from_checkpoint(...).generate(...)` one-liner works |
| M15 | Web interface (FastAPI + minimal frontend) | Local browser round-trip: prompt in, streamed text out |
