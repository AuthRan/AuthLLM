...
| M16 (stretch, needs GPU) | Scale to `medium`/`xl_1b` on rented GPU, real fp16/bf16 CUDA AMP + NCCL DDP | **Half done, on owned hardware rather than rented.** `medium` (124M) trained to completion from random init on 2.46B tokens of FineWeb-Edu -- 20,000 steps, ~27 hours, final validation perplexity 23.53 -- and the loss curve behaved as expected with no code changes to the CPU path, as predicted. Two predictions in this row were wrong in instructive ways: the run went **single-GPU, not NCCL DDP** (the second 2080 Ti is thermally throttled and DDP proceeds at the slower card's pace), and **fp16, not bf16** (Turing emulates bf16 at 7.7 TFLOP/s against fp16's 57.3 -- slower even than fp32). `xl_1b` was called "untrained and untrainable here" in this row for months, and half of that is now wrong: FSDP with CPU offload makes it take real optimizer steps at 2.04GB/GPU, against an OOM under DDP, under plain FSDP, and under FSDP with gradient checkpointing (`scripts/benchmark_fsdp.py`, logged in `logs/benchmark_fsdp.log`, table in README.md section 12.1). Sharding alone is not enough -- 1.23B parameters split two ways is still 2.5GB of fp32 weights plus gradients plus 9.9GB of AdamW state -- so what does it is keeping parameters and optimizer state in host RAM and streaming each layer to the GPU as it is reached, at roughly 10x the step time. `medium` measures 3.83GB/GPU under DDP against 1.67GB under FSDP at identical step time. **Untrained is still true**: 6.6 s/step at batch 1 is 1,024 tokens per step across both cards against the 122,880 the 124M run trained at, so matching that throughput is not a matter of waiting longer on this hardware |
...
| M19 (not in the original plan) | Multi-turn chat format | **Done.** `ashugpt/data/chat.py` makes a training document a conversation rather than a question: roles as plain-text markers (`### User:` / `### Assistant:`, not new special tokens -- a real role token means growing the vocabulary, which means a randomly initialized embedding row in a model whose other 50,257 rows carry 2.46B tokens of training), every assistant turn supervised and everything else masked, and `<|endoftext|>` ending a *turn* rather than the document. Long conversations are cut at a turn boundary rather than dropped or truncated mid-answer. Runs at seq_len 1024, not the 512 every other fine-tuning stage uses, and that is a data decision: at 512 only 10,203 of the 19,600 training conversations fit even their first answer and the mean survivor carries 1.45 assistant turns, which is a single-turn dataset wearing a chat template; at 1024, 17,672 survive and 77% carry two or more answers. The supervised fraction goes from ~11% (Alpaca) to 54%, which is also why packing is not wired up here -- there is almost no empty window left to reclaim. Learning rate swept over four points rather than argued by analogy (logs/sft_chat_lr*.csv): 1.0e-4 wins on held-out loss at 2.0333 against 2.1187 / 2.0630 / 2.0545 for 1.5e-5 / 4.0e-5 / 2.5e-4, which is 2.5x the value the config originally reasoned to and a bracketed minimum rather than the edge of the range. The full epoch at that rate is what ships and is a real improvement over the swept 300-step run, unlike the preference stage's epoch: held-out loss 1.9506 against 2.0333, a curve still falling at the last eval, and answers 195 tokens long against 222-token references -- against 8 points of stop rate, three generations in forty. Overall the stage moves held-out loss 0.79 nats, 2.8262 -> 1.9506, the largest single-stage move anywhere in this project. Two measurement findings outlived the stage. `scripts/eval_chat.py`'s first table said chat training had broken stopping (100% -> 57%) because it reused the single-turn eval's 200-token generation cap, and 71% of UltraChat's held-out answers are longer than that -- the untuned model "wins" by answering a 222-token question in 57 tokens. At a 400-token cap the same checkpoints stop 88-100% of the time, and the table now carries the human-written answers as a row because loop rate scales with answer length (15% of the real answers trip the same detector). And the turn leak rate this stage was designed around is 0% before *and* after: the instruction-tuned model never writes the user's next message, because one-answer-per-document taught it to emit EOS promptly -- so what multi-turn training buys is answering at the length and register a conversation calls for, not stopping in the right place. Cost on the single-turn distribution, same script and settings as section 10.4: 0.08-0.17 nats of Dolly held-out loss and answers 50-75% longer, which is that section's relocation effect a third time. README.md section 10.7; narrative in `results/chat-tuning.md`; diary in `learning/08-holding-a-conversation.md` |

| M20 (not in the original plan) | Preference tuning: teach it that one answer can be better than another | **Done, with a result worth more than the feature.** `ashugpt/training/dpo.py` (the DPO objective, the frozen reference, and a DPOModel wrapper that presents policy+reference as one model so trainer.train() is reused unchanged), `ashugpt/data/preference.py` (pairs as (2, seq_len), encoded through the same encode_supervised() the SFT datasets use, because the reference *is* the SFT model), `ashugpt/eval/preference.py` and `scripts/eval_preference.py`. Shipped: 400 steps at 1.0e-6 over 47,829 single-exchange HH-RLHF pairs, 32 pairs per step, ~15 min on one 2080 Ti. Measured on hh-rlhf's own test split (2,574 pairs): DPO accuracy 50% -> 57.1%, which is the objective and it generalizes. But **raw ranking accuracy -- the same question with no reference in it, so the SFT model can be scored on it too -- moved 46.3% -> 46.4%**, and splitting that by answer length explains why: the model ranks the chosen answer first 92.8% of the time when it is shorter and 8.3% when it is longer, before *and* after tuning. Summed log-probabilities cost 2-3 nats per token and HH-RLHF's chosen answers average 80 tokens against 73, so the base model is a length detector and DPO did not touch that; per-token accuracy, which divides the shortcut out, moved 54.3% -> 55.2%. Both implicit rewards go negative at every learning rate (the policy makes the *preferred* answers 12x less likely and the rejected ones 45x), which is invisible in the margin and is why the two are reported separately. The learning-rate sweep reproduced this project's recurring failure a fourth time: 2.0e-5 wins every preference metric and is the only checkpoint worse than its own starting model on every behavioural one (stop rate 92% -> 88%, mean answer 62 -> 103 tokens, loop rate 20% -> 40%). A full 1,495-step epoch was also run and is not what ships -- 3.7x the compute for 1.3 points of the trained objective and a drift the wrong way on behaviour. The length-normalized variant this row's own "what I would do differently" called for was then run at two betas (`configs/train/dpo_hh_ln10.yaml` and `dpo_hh_ln80.yaml`, 1.0 and 8.0, because normalizing shrinks the margins ~80x and 0.1 does not carry over), and **it does not fix the shortcut it was built for**: raw accuracy 46.3% -> 46.5% and the diagnostic split 92.8%/8.3% -> 93.0%/8.4%, i.e. marginally worse, after removing the length term from the objective entirely. The reason corrected a claim this project had been making: raw ranking accuracy scores a model by a *summed* log-probability, which grows with length whatever the weights are, so the shortcut lives in the ruler as much as in the model and no objective could have moved that column. Per-token accuracy, the column that already divides length out, is where it shows: 55.9% at beta 1.0 against standard DPO's 55.2% off 54.3%. The two betas split the usual way -- 8.0 wins the trained objective (57.9%) and ranks content worst of the three tuned checkpoints, 1.0 the reverse and is also the one that damages behaviour (88% stop, answers 39% longer), while 8.0 costs 0.0015 nats of Dolly loss and is the only checkpoint in the stage whose answers get shorter -- the fifth instance of that pattern here. Both implicit rewards still go negative under normalization at both betas, so the KL retreat is untouched. Nothing from the variant ships. README.md section 10.8; narrative in `results/preference-tuning.md`; diary in `learning/07-preference-tuning.md` |
| M21 (not in the original plan) | Figures: draw the curves, and check the drawings against the measurements | **Done, and the checking is the part that mattered.** `ashugpt/viz/` (`logs.py` parses the five-column CSV every trainer here writes, `style.py` holds one visual language, `figures.py` is one function per figure) and `scripts/plot_results.py` shipped five figures built from files already in the tree. `figures.py` said the arrangement meant "a figure and its prose cannot drift apart silently", and nothing checked that, so this milestone is the check: `tests/unit/test_viz_figures.py` reads the same `results/*.md` tables the scoring scripts wrote and asserts the hand-copied numbers still agree with them, and `tests/unit/test_viz_logs.py` covers the parser, which was the only module under `ashugpt/` with no tests at all. Both drifts it found were already shipped in the README's images. **Figure 05 drew the 5.0e-6 DPO checkpoint at 94% stop / 30% loop / 88 tokens against a measured 98% / 22% / 79** -- a checkpoint reported as behaving worse than it does, in the one figure whose whole argument is that two rankings disagree; the argument survives (loop rate and answer length still climb monotonically with learning rate) but the middle bar was wrong. **Figure 02 drew `tiny` at 12.3M and `small` at 33.9M against configs saying 7,292,032 and 29,938,560** -- 69% and 13% over, and both numbers are derivable, so those three columns are now read from `configs/model/*.yaml` through the same `approx_param_count()` README section 4 quotes rather than typed. A third bug was in the parser and had not reached a figure yet: `load_log` skipped the append for a blank perplexity instead of padding it the way `lr` already padded, so `val_perplexity[i]` stopped meaning `val_steps[i]` -- and the trainer writes an empty perplexity deliberately whenever the loss is not a cross-entropy, so all six DPO logs in `logs/` were already parsing to 15 validation losses and 0 perplexities. Nothing had zipped the two, which is the only reason no figure was wrong. Fixed by padding with NaN, plus a `final_val_perplexity` property, because a NaN is truthy and the obvious guard would have annotated a figure with the string "nan". Checking section 4 against the figure also caught stale prose: its "Trained here?" column still said `medium` was "shape/forward-pass tested only", 200 lines below the same README calling it **trained to completion**. `matplotlib` moves into `requirements.txt` so the guard runs in CI; it stays an optional extra in `pyproject.toml`, because nothing under `ashugpt/` trains, evaluates or serves through it. **Extended the same day to the commands**, since `test_doc_links.py` checked links and nothing checked what a reader copies: `tests/unit/test_doc_commands.py` asserts every tracked path in a fenced block exists, and that a cited `step_N.pt` is a step the config named in the same block reaches. It found README section 15 training with `configs/train/dpo_hh.yaml` (`max_steps: 400`) and then evaluating `checkpoints/dpo_hh/step_1495.pt` -- the full-epoch run this milestone's own row says was measured and not shipped, while `results/preference-tuning.md` cited `step_400.pt` for the same stage. No missing file, no dead link, correct prose around it. Checkpoints are compared against the config that would produce them rather than against disk, because `checkpoints/` and `data/` are gitignored and absent from the clean checkout CI runs from. SPEC.md is excluded from the scan: its Commands and Project Structure blocks are the original design spec, and asserting a pre-code plan matches the tree would assert the project never learned anything. |
| M22 (not in the original plan) | Sampling with a memory, and serving a checkpoint in its own format | **Done, with the same shape of result as every stage before it.** Two defects that made a working model look broken. First, the frontend always opened on "Continue text", so the chat checkpoint was asked to continue a question rather than answer it -- nothing in a checkpoint file records what it was fine-tuned on, so it is declared now (`scripts/serve.py --format {base,instruct,chat}` -> `ASHUGPT_FORMAT` -> `/health` -> the page's opening tab), defaulting to `base` because continuation is the one answer that is never actively wrong, and refusing to start on an unknown value. Second, `ashugpt/inference/generate.py` gained `apply_repetition_penalty` (Keskar et al. 2019: divide a positive logit, multiply a negative one -- dividing a negative logit would make a disfavoured token *more* likely), the first sampling filter here that looks backwards rather than at one step's logits. Loop rate had been 18-40% across every checkpoint in section 10 and no training stage could have fixed it. Swept on `sft_dolly_packed3e5/step_940.pt`, same split and settings as section 10.4: **1.0 (off) 92% stop / 62 tok / 20% loop; 1.1 98% / 74 / 8%; 1.2 88% / 102 / 0%; 1.5 48% / 154 / 0%**. Held-out loss is 2.7444 at every row -- teacher-forced loss never samples, so the metric this project usually reaches for cannot see the change at all. 1.1 is the only setting that improves both, and past it the recurring failure appears a sixth time: the targeted metric reaches 0% and stays there while stopping collapses, because the tokens a model needs in order to end are common, therefore repeated, therefore penalised. Defaults to 1.0 in the library, the API and the eval script so every published number stays comparable; the frontend slider defaults to 1.1 because it is a demo rather than a measurement. The eval preamble records the penalty a table was produced under, so two tables at different settings cannot be read as one comparison. `sample_next_token` deliberately does *not* take the parameter -- it sees one step's logits and has no history, so the argument would be a silent no-op on a public function. Narrative in `results/repetition-penalty.md`; README sections 11.2 and 18. |
