...
| M17 (not in the original plan) | Instruction tuning: teach the base model to answer rather than continue | **Done.** `ashugpt/data/instruction.py` (template + EOS + prompt masking via `ignore_index=-100`), `scripts/finetune.py` (weights-only init, fresh optimizer), `scripts/eval_instruction_following.py` (held-out loss + stop rate + loop rate). Two stages ran on the 124M checkpoint: Alpaca 52k at 2e-5, then Dolly 15k at 1e-5. Measured, on held-out data neither stage trained on: stop rate 30% -> 100%, loop rate 80% -> ~15%, mean answer length 179 -> ~52 tokens. Two findings worth more than the headline: three epochs of Alpaca produce a model whose *behavioural* metrics are perfect and whose held-out loss is worse than the base model it started from, and every epoch of the Dolly stage trades Alpaca held-out loss for Dolly held-out loss at close to 1:1 -- stage 2 relocates the model rather than improving it. See README.md section 10 and `results/instruction-tuning.md` |
